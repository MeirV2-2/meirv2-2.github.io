...
Run Moneta against a vanilla Windows 11 workstation with Citrix Workspace installed. Citrix's ctxapinject.sys kernel driver loads ctxapclient64.dll into nearly every interactive process on the system, and that DLL allocates `MEM_PRIVATE` + `PAGE_EXECUTE_READWRITE` regions for its API-hook trampolines. Moneta flags every one of those regions with the same 'abnormal private executable memory' message — across every process. Not just Citrix's own processes: the artifact propagates to whatever Citrix has injected into, which on a managed enterprise box is essentially everything. So does Adobe Reader's protected-mode broker. So does the .NET CLR's JIT-emitted code in any managed application. So does V8's stub code in Chrome's renderer.
